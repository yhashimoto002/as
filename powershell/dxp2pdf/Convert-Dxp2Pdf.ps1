...
process
{
    ...
    {
        Get-ChildItem $sourceFiles | ? { $_.FullName -match $docRegex } | Convert-WordToPdf
        Get-ChildItem $sourceFiles | ? { $_.FullName -match $excelRegex } | Convert-ExcelToPdf
        Get-ChildItem $sourceFiles | ? { $_.FullName -match $powerpointRegex } | Convert-PowerPointToPdf
    }
    if($Word)
    {
        Get-ChildItem $sourceFiles | ? { $_.FullName -match $docRegex } | Convert-WordToPdf
