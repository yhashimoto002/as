...
# if specified no $outDir, output the same directory as input
...
function Convert-Word
{
    ...
    process
    {
        ...
        $pdfFilePath = (Join-Path $outDir $Object.Name) + ".pdf"
        $result = ""
        $errMessage = ""
...
function Convert-PowerPoint
{
    ...
    process
    {
        ...
        catch
        {
            ...
            elseif ($_.Exception.Message -match "HRESULT E_FAIL")
            {
                $errMessage = "ファイル破損"
            }
            else
...
# main
$startTime = Get-Date
dir $inDir | ? { $_.FullName -match $docRegex } | Convert-Word
dir $inDir | ? { $_.FullName -match $excelRegex } | Convert-Excel
dir $inDir | ? { $_.FullName -match $powerpointRegex } | Convert-PowerPoint
