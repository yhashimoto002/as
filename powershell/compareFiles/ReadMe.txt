...
2 つのフォルダにある同じ名前の PDF の見た目を比較し、結果を csv で出力します。
csv には以下のカラムが出力されます。
...
1. comparePdf フォルダごと適当な場所にコピーします。
...
5. Compare-Pdf.ps1 が置いてあるパスに移動します。以下は c:\work に置いた場合です。

> cd c:\work

5. Compare-Pdf.ps1 に比較したい PDF が入っているフォルダをそれぞれ引数で指定して実行します。

> .\Compare-Pdf.ps1 .\before .\after

※PDF だけを選んで比較するので、PDF 以外のファイルが混ざっていても特に問題はありません。

6. 処理が終わるとスクリプトが置いてあるフォルダに result.csv が出力されます。

7. result.csv を Excel で開きます。Result カラムが NG の PDF/ページを探してください。
...
PS> Unblock-File .\sds1-check.ps1
